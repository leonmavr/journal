...
pid_t fork(void);
\end{verbatim}
\marginnote{Fork's return indicates failure or not.}To differentiate whether its called for the parent or child, it returns different values for each, particularly:
% see https://www.geeksforgeeks.org/fork-system-call/
\begin{enumerate}
...
\begin{exmp}
\textup{
\lstinputlisting[language=c,caption={Accessing the same variable with two processes (\detokenize{src/fork_values2.c)}.}, label=src:mylabel]{src/fork_values2.c}
} % textup
\end{exmp}
\begin{verbatim}
...
% ref https://richard.esplins.org/static/downloads/linux_book.pdf p74
...
\begin{exmp}
\textup{
\lstinputlisting[language=c,caption={Status code of multiple children (\detokenize{src/fork_sync.c)}.}, label=src:mylabel]{src/fork_sync.c}
}
\end{exmp}
Output:
\begin{verbatim}
